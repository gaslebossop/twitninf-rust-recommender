...
impl UserDimensionWeights {
    ...
    fn for_user_type(user_type: &UserType) -> Self {
        match user_type {
            ...
            UserType::PowerUser => Self {
                d1: 0.35, d2: 0.14, d3: 0.12, d4: 0.10,
                d5: 0.09, d6: 0.05, d7: 0.11, d8: 0.04,
            },
            // Regular: poids Phase 1 par défaut (bien calibrés)
            UserType::Regular => Self {
                d1: 0.32, d2: 0.18, d3: 0.15, d4: 0.10,
                d5: 0.08, d6: 0.06, d7: 0.07, d8: 0.04,
            },
            // Casual: réseau social fort + fraîcheur (moins de pression virale)
            UserType::Casual => Self {
                d1: 0.25, d2: 0.20, d3: 0.20, d4: 0.13,
                d5: 0.08, d6: 0.06, d7: 0.05, d8: 0.03,
            },
        }
    }
    ...
    fn normalize(&mut self) {
        let sum = self.d1 + self.d2 + self.d3 + self.d4
            + self.d5 + self.d6 + self.d7 + self.d8;
        if sum > 0.0 {
            self.d1 /= sum; self.d2 /= sum; self.d3 /= sum; self.d4 /= sum;
            self.d5 /= sum; self.d6 /= sum; self.d7 /= sum; self.d8 /= sum;
        }
    }
    ...
    pub fn apply(
        ...
        let score = d1 * self.d1 + d2 * self.d2 + d3 * self.d3 + d4 * self.d4
            + d5 * self.d5 + d6 * self.d6 + d7 * self.d7 + d8 * self.d8;
        trace!(
            score, d1, d2, d3, d4, d5, d6, d7, d8,
            w1=self.d1, w2=self.d2, w3=self.d3, w4=self.d4,
            w5=self.d5, w6=self.d6, w7=self.d7, w8=self.d8,
...
mod tests {
    ...
    fn test_poweruser_has_higher_d1() {
        ...
        assert!(power.d1 > casual.d1, "PowerUser should weight D1 more than Casual");
    }
    ...
    fn test_casual_has_higher_d3() {
        let power = UserDimensionWeights::for_user_type(&UserType::PowerUser);
        let casual = UserDimensionWeights::for_user_type(&UserType::Casual);
        assert!(casual.d3 > power.d3, "Casual should weight social graph more");
    }
}
