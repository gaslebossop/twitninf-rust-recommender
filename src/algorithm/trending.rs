...
pub fn trending_score(t: &RawTweet) -> f64 {
    ...
    trace!(age_h, total_eng, eng_per_hour, "Trending: engagement velocity");

    // Bonus pour tweets très récents avec déjà de l'engagement
    let recency_mult = if age_h < 1.0 { 3.0 }
        else if age_h < 3.0 { 2.0 }
        else if age_h < 6.0 { 1.5 }
        else { 1.0 };
...
pub fn top_trending(tweets: &[RawTweet], top_n: usize) -> Vec<(String, f64)> {
    debug!("Calculating trending scores for {} tweets, top {}", tweets.len(), top_n);
    let mut scores: Vec<(String, f64)> = tweets.iter()
        .map(|t| {
    ...
    debug!("Top trending tweets: {:?}", scores.iter().map(|(id, score)| (id.clone(), format!("{:.4}", score))).collect::<Vec<_>>());
    scores
}
