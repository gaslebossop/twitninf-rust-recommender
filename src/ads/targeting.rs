...
impl AdTargetingEngine {
    ...
    pub fn score_ad(
        ...
        reasons.push(MatchReason {
            signal: "keyword_match".into(),
            weight: W_KEYWORD,
            score: kw_score,
        });
        reasons.push(MatchReason {
            signal: "user_segment".into(),
            weight: W_SEGMENT,
            score: seg_score,
        });
        reasons.push(MatchReason {
            signal: "temporal_alignment".into(),
            weight: W_TEMPORAL,
            score: tmp_score,
        });
        reasons.push(MatchReason {
            signal: "engagement_potential".into(),
            weight: W_ENGAGE,
            score: eng_score,
        });
        reasons.push(MatchReason {
            signal: "revenue_potential".into(),
            weight: W_REVENUE,
            score: rev_score,
        });
    ...
    pub fn select_ads(
        ...
        let mut scored: Vec<ScoredAd> = campaigns
            .iter()
    ...
    fn keyword_score(
        ...
        let matched_weight: u32 = keywords
            .iter()
    ...
    fn segment_score(&self, segments: &[AudienceSegment], profile: &UserProfile) -> f64 {
    ...
    fn temporal_score(
        ...
        if let Some(spec) = time_targeting {
            let hour_ok = spec.hours.is_empty() || spec.hours.contains(&context.current_hour);
            let day_ok =
                spec.weekdays.is_empty() || spec.weekdays.contains(&context.current_weekday);
            if !hour_ok || !day_ok {
    ...
    fn engagement_score(&self, profile: &UserProfile) -> f64 {
        ...
        let trend_bonus = if profile.engagement_trend > 0.0 {
            0.10
        } else {
            0.0
        };
